...
public class HibernateToDoListDAO implements IToDoListDAO , IToDoListUser{
	...
	public User getUserByEmail(String email) throws TodoListPlatformException {
		...
		try{
			...
			session.beginTransaction();
			//Query for select Id(return only Id) by email
			Query query = session.createQuery("select id from User as u where u.email= :user_email");
			...
			{
				session.getTransaction().commit();
				user = (User) session.get(User.class, userIdList.get(0));
